...
function* store({payload: {ladder, name, alias, description}}): SagaIterator {
    try {
        const response = yield call(storeApi, {ladder, name, alias, description});
        const data = response && response.data.result;
...
function* update({payload: {id, ladder, name, alias, description}}): SagaIterator {
    try {
        const response = yield call(updateApi, {id, ladder, name, alias, description});
        const data = response && response.data.result;
